...
const OrgRolesApproval = () => {
    ...
    async function getRoleTree() {
        ...
        if (data === undefined) {
            ...
            setRoleServiceTree(<TreeDataViewer data={makeRoleServiceTree(data)} checkable={true} update={onCheckRoles}
            ...
            .then((data) => {
                if (data.length > 0) {
                    ...
                    setMappingImpossibleList(
                        <>
                            <div style={{fontWeight: 'bold', fontSize: '17px'}}>{`조직&역할(기본) 에 설정되어있음`}</div>
                            <br/>
                            {data.map((o) => {
