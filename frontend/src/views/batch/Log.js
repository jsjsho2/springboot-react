...
const Log = () => {
    ...
    useEffect(() => {
        setCalender(<Calendar dateCellRender={fn.dateCellRender} onPanelChange={(date) => {
            fn.getData(fn.dateRangeSetting(new Date(date)));
        }}/>);
    }, [batchLogList]);
    useEffect(() => {
        fn.getData(fn.dateRangeSetting(new Date()));
    }, []);
    ...
    const fn = {
        ...
        dateCompare: (batchLogList, value) => {
            let list = [];
        ...
        getListData: value => {
            ...
            if (batchLogList.length !== 0) {
                listData = fn.dateCompare(batchLogList, value)
            }

            return listData || [];
        },
        dateCellRender: value => {
            const listData = fn.getListData(value);

            return (
                <ul className="events">
                    {listData.map(item => (
                        <li key={common.createUuid()}>
                            <Badge status={item.type} text={item.content} uuid={item.uuid} style={{width: 'unset'}}
                                   onClick={e => {
