...
const List = () => {
    ...
    function batchVisibleFunc(visible) {
        setBatchVisible(visible);
    }

    function addBatchVisibleFunc(visible) {
        setAddBatchVisible(visible);
    }

    const StateCustomColor = (props) => {
        let statusTag;
        switch (props.value) {
            case 0:
    ...
    const batchInfoButtons = (
        <div key={'buttons'}>
            <Button onClick={() => {
                batchVisibleFunc(false)
            }}>닫기
            </Button>
            <Popconfirm
                title="저장된 내용으로 배치를 수동 실행하시겠습니까?"
                onConfirm={manualBatchInit}
                okText="예"
                cancelText="아니오"
            >
    ...
    const frameworkComponents = {
        stateCustomColor: StateCustomColor,
        usageCustomColor: UsageCustomColor,
        details: Details
    };
    const colOption = [
